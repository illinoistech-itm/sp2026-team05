...
import { ArrowLeft, Send, ChevronDown, MessageCircle } from "lucide-react";
        ...
        <div className="messages-list-card">
          ...
          <div className="messages-list-items">
            {mockConversations.slice(0, visibleCount).map((conv) => (
              <div key={conv.id} className="messages-list-item" onClick={() => openConversation(conv)}>
          ...
          {visibleCount < mockConversations.length && (
            <div className="messages-show-more">
              <button className="messages-show-more-btn" onClick={() => setVisibleCount((v) => v + 4)}>
                <ChevronDown size={28} />
              </button>
            </div>
          )}
